...
// New requests get an HTTP 500 instead of running the handler. Existing
...
function handleSimulated(req, url, match) {
  ...
  (async () => {
    ...
    if (killed) {
      ...
      if (errored) {
        responseType = 'error';
        resolveResponse(new Response('Internal error', { status: 500 }));
      } else {
    ...
    accessLogger?.({ phase: 'end', method, url, durationMs: Date.now() - startedAt, eventCount, aborted, responseType });
  })();
